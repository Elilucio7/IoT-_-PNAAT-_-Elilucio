...
flag = False #flag para alarme manual
...
while True: #loop do programa
  ...
  if temp > 99 and temp <125 and not flag: #ativar alerta de aquecimento
  ...
  if temp == 125 or temp > 125 and not flag: #ativar alarme de temperatura muito elevada
    led_temp_pin.value(1)
    led_evac_pin.value(1)
    oled.fill(0)
    oled.text(f"ALERT, HIGH TEMP", 0, 0)
  
  if temp < 100 and not flag: #desativar alarme quando temperatura estiver abaixo de 100 graus
